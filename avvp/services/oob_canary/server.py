"""
Out-of-Band (OOB) Canary Server — detects blind SSRF, SQLi, RCE via callback detection.

Lightweight aiohttp server that:
- Listens on configurable port (default 8877)
- Accepts any GET/POST to any path
- Extracts token from path: /{scan_id}/{finding_id}/{random}
- Stores callbacks in memory: {token → {timestamp, source_ip, method, path}}
- Exposes GET /check/{token} endpoint to query callback status
...
class OOBCanaryServer:
    ...
    Stores all callbacks in memory; suitable for single-scan operations.
    """
    ...
    async def _handle_request(self, request: web.Request) -> web.Response:
        ...
        callback = Callback(
            ...
            body=body[:500],  # Truncate to 500 chars
        )
