"""LFI validation with reward feedback."""
...
SIGNATURES = ("root:", "daemon:", "[boot loader]", "nobody:")
...
@register("lfi")
async def validate_lfi(url: str, param: str, timeout: int = 20) -> dict | None:
    parsed = urlparse(url)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        payloads = suggest_payloads("lfi", n=15) or PAYLOADS
        for payload in payloads:
            ...
            if any(signature.lower() in body.lower() for signature in SIGNATURES):
                _engine.record_result(payload, "lfi", reward=1.0, waf=waf, tech=[])
                return {
                    "validated": True,
                    "type": "LFI",
                    "url": test_url,
            ...
            _engine.record_result(payload, "lfi", reward=reward, waf=waf, tech=[])
