...
Replaces the shallow genetic-only approach with:
  - Form discovery and POST parameter fuzzing
  - Broad parameter coverage (all detected param_patterns from signal bag)
  - Error-based detection with Java/Tomcat/MySQL/Oracle patterns
...
class SQLiValidator(BaseValidator):
    ...
    def run(self, state: Dict[str, Any]) -> Optional[ValidationResult]:
        ...
        # ── PART C Enrichment: re-test /bank/queryxpath.jsp with XPath payloads using auth session
        ...
        # Run checks on unique targets (cap at 25 for execution speed)
