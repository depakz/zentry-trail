...
Replaces genetic-only approach with:
  - Form discovery and POST/GET form parameter fuzzing
  - Character reflection detection (unescaped <, >, ", ')
  - HTML tag injection confirmation
  - Broad parameter coverage from signal bag
  - Auth-aware (uses session cookies from state)
"""
...
class XSSValidator:
    ...
    def run(self, state: Dict[str, Any]) -> Optional[ValidationResult]:
        ...
        # ── PART C Enrichment: re-test /feedback.jsp (stored XSS) and /search.jsp with auth session
        ...
        # ── 1. Discover Forms on Pages ─────────────────────────────────────────
        forms = self._discover_forms(target_url, endpoints, cookies)

        # ── 2. Build Unified Test Targets ──────────────────────────────────────
        test_targets = []
        
        # Add form-based targets
        for f in forms:
        ...
        # Deduplicate targets by (path, param, method)
        seen = set()
        dedup_targets = []
        ...
        # Prioritize targets containing high priority parameters
        dedup_targets = sorted(
            dedup_targets,
            key=lambda x: (0 if x["param"].lower() in HIGH_PRIORITY_PARAMS else 1, x["url"])
        )

        # Run checks on unique targets (cap at 25 for speed)
    ...
    def _test_target(self, target: Dict[str, Any], cookies: dict) -> Optional[ValidationResult]:
        ...
        if probe_val in body:
            # Check unescaped dangerous chars
            ...
            if "<" in unescaped or ">" in unescaped:
                ...
                if "zentry_xss_tag" in tag_body and "<b>" in tag_body:
                    ...
                    return ValidationResult(
                        ...
                        vulnerability="reflected-xss",
                        evidence=Evidence(
                            request={"target": evidence_url, "param": param, "payload": HTML_PAYLOADS[0], "method": method, "post_data": tag_fields if method == "POST" else None},
                            response={"snippet": tag_body[:500]},
                            matched=HTML_PAYLOADS[0],
                        ),
                ...
                # Unescaped < > without tag confirmation still high-confidence
                if unescaped:
                    ...
                    return ValidationResult(
                        ...
                        evidence=Evidence(
                            request={"target": evidence_url, "param": param, "payload": danger_val, "method": method, "post_data": danger_fields if method == "POST" else None},
                            response={"snippet": danger_body[:500]},
                            matched=str(unescaped),
                        ),
        ...
        # ── 2. Script payload direct injection ────────────────────────────────
        for payload in JS_PAYLOADS:
            ...
            if "<script>" in body.lower() and "alert" in body.lower():
                ...
                return ValidationResult(
                    ...
                    evidence=Evidence(
                        request={"target": evidence_url, "param": param, "payload": payload, "method": method, "post_data": test_fields if method == "POST" else None},
                        response={"snippet": body[max(0, snippet_idx - 50): snippet_idx + 200]},
                        matched=payload,
                    ),
