"""Genetic Fallback Engine Validator for XSS."""
...
class XSSValidator:
    ...
    def run(self, state: Dict[str, Any]) -> Optional[ValidationResult]:
        ...
        params = state.get("injection_params", ["id", "q", "search"])
        ...
        try:
            for param in params:
                ...
                # Hooking genetic engine fallback into xss_validator when metrics yield < 0.3 fitness.
                def xss_evaluator(payload: str):
                ...
                best_payload, best_score = self.engine.evolve(
                    "XSS", target_url, param, xss_evaluator, baseline_resp, remaining_budget_seconds=30.0
                )
                
                if best_score >= 0.35:
                    return ValidationResult(
                        success=True, confidence=best_score, severity="high", vulnerability="xss-genetic-evolved",
                        evidence=Evidence(request={"url": target_url, "param": param}, response={"fitness": best_score}, matched=best_payload),
                        impact="XSS found via genetic payload evolution.", remediation="Use context-aware output encoding."
                    )
