...
class AuthManager:
    ...
    def login(self, login_url: str, user_field: str, pass_field: str) -> bool:
        ...
        Success detection:
          - Response does NOT contain login form indicators
            (password input, user_field, pass_field, "Invalid username")
          - Response contains post-login indicators
            ("My Account", "Sign Off", "Welcome")
        ...
        try:
            ...
            # Extract form action
            post_url = login_url
            action_m = re.search(r'action=["\']([^"\']+)["\']', r_get.text, re.I)
            if action_m:
            ...
            # Parse hidden inputs
            for hm in re.finditer(r'<input[^>]+type=["\']hidden["\'][^>]*>', r_get.text, re.I):
            ...
            # Detect success by checking response content
            body_lower = r.text.lower()

            # Check for login form still present (failure indicator)
            has_login_form = (
                re.search(r'<input[^>]+type=["\']?password', r.text, re.I)
                or f'name="{user_field}"' in r.text
                or f"name='{user_field}'" in r.text
            )
